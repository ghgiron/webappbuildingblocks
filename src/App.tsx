...
export default function App() {
  ...
  return (
    ...
    >
      {/* ========================================================================= */}
      {/* 1. HEADER INSTITUCIONAL (NAVEGACIÓN EN LA MISMA VENTANA: target="_self")   */}
      {/* ========================================================================= */}
      ...
      {isLandscape ? (
        ...
        <div className="flex-1 min-h-0 w-full max-w-[1920px] mx-auto p-3 lg:p-4 grid grid-cols-12 gap-4 overflow-hidden items-stretch">
          ...
          <section className={`col-span-3 h-full flex flex-col bg-white border border-slate-200 rounded-[15px] p-4 shadow-2xs overflow-hidden transition-all duration-500 ${isCompleted ? 'opacity-85' : 'opacity-100'}`}>
          ...
          <section className="col-span-6 h-full flex flex-col items-center justify-center relative p-2 overflow-hidden">
            ...
            <div className="flex-1 min-h-0 w-full flex items-center justify-center relative max-h-[100%] aspect-[800/1000]">
              ...
              >
                ...
                {/* Outer Red Silhouette of the House of Bogotá (Con marco perimetral visible) */}
                <path
      ...
      {/* ========================================================================= */}
      {/* 3. MODAL GLOBAL DE FELICITACIONES (CENTRADO ABSOLUTO EN TODAS LAS PANTALLAS) */}
      {/* ========================================================================= */}
      {isCompleted && showCelebrationModal && !isSimulationMode && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="relative w-full max-w-md bg-white rounded-[15px] border-2 border-[#FAB62D] p-6 sm:p-7 shadow-2xl text-center flex flex-col items-center gap-4 z-10 animate-in zoom-in-95 duration-200 text-[#333333]">
            {/* Botón de cierre "✕" en la esquina superior derecha */}
            <button
              onClick={() => setShowCelebrationModal(false)}
              className="absolute top-3 right-3 w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 flex items-center justify-center transition-all cursor-pointer"
              title="Cerrar y Ver Casa Armada"
            >
              <X className="w-5 h-5" />
            </button>

            {/* Logo oficial de la Alcaldía ("Aquí sí pasa, Bogotá, mi ciudad, mi casa") */}
            <img
              src="/assets/logo-campana.png"
              alt="Aquí Sí Pasa - Bogotá Mi Ciudad, Mi Casa"
            ...
            {/* Título y Mensaje Institucional Estricto */}
            <div className="flex flex-col gap-1.5">
              <h3 className="font-title text-2xl sm:text-3xl font-extrabold text-[#333333] tracking-tight">
                ¡Felicitaciones!
              </h3>
              <p className="font-body text-xs sm:text-sm font-normal text-[#555555] max-w-sm mx-auto leading-relaxed">
                Has construido Bogotá con las 15 soluciones compartidas (Building Blocks) que hacen posible un Distrito ágil e interoperable para <span className="font-bold text-[#CC0E35]">la ciudadanía</span>.
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-2.5 pt-1 w-full justify-center">
              <button
                ...
                className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-3 bg-[#FAB62D] hover:bg-[#CC0E35] text-[#333333] hover:text-white font-title font-extrabold text-xs sm:text-sm rounded-[15px] shadow-md transition-all active:scale-95 cursor-pointer"
              >
              ...
              <button
                onClick={() => setShowCelebrationModal(false)}
                className="w-full sm:w-auto px-4 py-3 bg-slate-100 hover:bg-slate-200 active:scale-95 text-slate-700 font-title font-bold text-xs sm:text-sm rounded-[15px] border border-slate-200 transition-all cursor-pointer"
              >
                Ver Casa Armada
              </button>
            </div>
      ...
      {/* ========================================================================= */}
      {/* 4. MODAL / BOTTOM SHEET DE CONCEPTO Y EJEMPLO (CONSULTA ON-DEMAND)        */}
      {/* ========================================================================= */}
      {showConceptModal && (
      ...
      {/* ========================================================================= */}
      {/* 5. FLOATING DRAGGED PIECE AVATAR (Follows touch pointer)                  */}
      {/* ========================================================================= */}
